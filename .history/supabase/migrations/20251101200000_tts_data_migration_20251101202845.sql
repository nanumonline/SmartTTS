...
-- 개발 환경에서는 RLS를 완화하여 인증되지 않은 사용자도 UUID로 접근 가능
...
ALTER TABLE public.tts_voice_catalog ENABLE ROW LEVEL SECURITY; -- 공개 읽기, 공개 쓰기

-- RLS 정책: 인증된 사용자는 자신의 데이터, 비인증 사용자는 UUID로 접근 가능
-- 개발 환경에서 더미 사용자도 동작하도록 완화 (프로덕션에서는 실제 인증 필요)
...
-- 나머지 RLS 정책들도 기존 정책 삭제 후 새로 생성
DO $$
BEGIN
  -- tts_generations 정책들 (인증되지 않은 사용자도 UUID로 접근 가능)
  DROP POLICY IF EXISTS "Users can insert own generations" ON public.tts_generations;
  DROP POLICY IF EXISTS "Users can update own generations" ON public.tts_generations;
  DROP POLICY IF EXISTS "Users can delete own generations" ON public.tts_generations;
END $$;
...
-- tts_voice_catalog 정책들 (공개 읽기, 공개 쓰기)
