...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Usage/Credit 조회 여부 확인
    const isUsage = pathAfterFunction.startsWith("/usage");
    const isCredit = pathAfterFunction.startsWith("/credit-balance");

    // Usage/Credit의 경우 GET만 허용
    if ((isUsage || isCredit) && req.method !== "GET") {
      return new Response(
    ...
    // Supertone API 호출 URL (공식 레퍼런스: https://docs.supertoneapi.com/en/api-reference/introduction)
    const supertoneBaseUrl = "https://supertoneapi.com/v1";
