...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const loadUserProfile = async (authUser: any, skipLoadingState = false) => {
    ...
    try {
      // 프로필 조회 시도 (없어도 에러로 처리하지 않음)
      // maybeSingle()을 사용하여 프로필이 없어도 에러가 발생하지 않음
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      // loadUserProfile이 로딩 상태를 관리하도록 함 (skipLoadingState: true로 설정하여 중복 방지)
      // 하지만 인증 단계에서 이미 setIsLoading(true)를 호출했으므로, 프로필 로드 중에도 로딩 상태 유지
      try {
