  ...
  useEffect(() => {
    ...
    const validateBlobUrl = async (url: string): Promise<boolean> => {
      ...
      try {
        ...
        // blob URL이 유효한지 fetch로 확인 (HEAD 요청으로 빠르게 확인)
        const response = await fetch(url, { method: 'HEAD', cache: 'no-cache' });
        return response.ok;
      } catch (e) {
        console.warn('Blob URL validation failed:', url, e);
        return false;
      }
    };
    ...
    if (audioUrl && audioUrl.trim() !== '') {
      ...
      if (audioUrl.startsWith('blob:')) {
        validateBlobUrl(audioUrl).then((isValid) => {
          if (!isValid && onError) {
            // 유효하지 않은 blob URL이면 즉시 재생성 시도
            console.warn('Invalid blob URL detected, attempting recovery:', audioUrl);
            onError();
            return;
          }
          // 유효한 경우에만 src 설정
    ...
    const handleError = (e: any) => {
      ...
      const isRangeError = error?.code === 4 || 
          error?.message?.includes('RANGE_NOT_SATISFIABLE') ||
          error?.name === 'NotSupportedError' ||
          audio?.networkState === 3 || // NETWORK_NO_SOURCE
          audio?.readyState === 0; // HAVE_NOTHING
      
      if (isBlobUrl && isRangeError) {
        console.warn('Blob URL expired or invalid (ERR_REQUEST_RANGE_NOT_SATISFIABLE):', audioUrl, error);
        // 즉시 복원 시도 (onError 콜백 호출)
        if (onError) {
          // src를 비워서 추가 에러 방지
          if (audio) {
            audio.src = '';
        ...
        console.warn('Blob URL error detected:', audioUrl, error);
        if (onError) {
          // src를 비워서 추가 에러 방지
          if (audio) {
            audio.src = '';
