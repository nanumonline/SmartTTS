...
export default function MessageManagementPage() {
  ...
  // 지침 토글 상태 추적
  const [hasBasicPromptInstruction, setHasBasicPromptInstruction] = useState(false);
  const [hasPublicGuidelinesInstruction, setHasPublicGuidelinesInstruction] = useState(false);
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      ...
      if (editingMessageId) {
        // 수정
        if (user?.id) {
          image.png          await dbService.updateMessage(user.id, editingMessageId, customText, tags);
        }
        const updated = messageHistory.map((m) =>
          m.id === editingMessageId
            ? { ...m, text: customText, tags: tags, updatedAt: new Date().toISOString() }
            : m
        );
      ...
      setHasPublicGuidelinesInstruction(false);
    } catch (error) {
  ...
  // openAIInstruction 변경 시 지침 상태 업데이트
  useEffect(() => {
    const instruction = openAIInstruction.trim();
    const basicInstruction = purposeMetaForSelected?.optimizedPrompt || "";
    
    // 기본 프롬프트 지침 포함 여부 확인
    const hasBasic = instruction.includes(basicInstruction) && basicInstruction.length > 0;
    // 공공기관 방송지침 포함 여부 확인
    const hasPublic = instruction.includes("공공기관(지자체) 방송지침:");
    
    setHasBasicPromptInstruction(hasBasic);
    setHasPublicGuidelinesInstruction(hasPublic);
  }, [openAIInstruction, purposeMetaForSelected?.optimizedPrompt]);
                    ...
                    return (
                      <Card
                        ...
                        onDoubleClick={() => {
                          ...
                          if (purposeMeta.optimizedPrompt) {
                            setActiveTab("manual");
                            setCustomText(purposeMeta.optimizedPrompt);
                            toast({
                              title: "기본문구 적용 완료",
                              description: `${option.label}의 기본문구가 직접작성에 적용되었습니다.`,
                            });
                          }
                        }}
                      >
                            ...
                            <Accordion type="single" collapsible className="w-full">
                              <AccordionItem value="guidelines" className="border-none">
                                <div className="flex items-center justify-between">
                                  ...
                                  <Button
                                    variant={hasPublicGuidelinesInstruction ? "default" : "outline"}
                                    size="sm"
                                    ...
                                    onClick={() => {
                                      ...
                                      if (!hasPublicGuidelinesInstruction) {
                                        ...
                                        setOpenAIPrompt(baseText);
                                        if (hasBasicPromptInstruction) {
                                          // 기본 프롬프트 지침이 이미 있으면 함께 유지
                                          setOpenAIInstruction(`${basicInstruction}\n\n${guidelinesText}`);
                                        } else {
                                          // 기본 프롬프트 지침이 없으면 공공기관 지침만 추가
                                          setOpenAIInstruction(guidelinesText);
                                        }
                                        setHasPublicGuidelinesInstruction(true);
                                        setActiveTab("ai-assist");
                                        toast({
                                          title: "지침 추가 완료",
                                          description: "공공기관 방송지침이 추가되었습니다.",
                                        });
                                      } else {
                                        // 공공기관 지침 제거
                                        if (hasBasicPromptInstruction) {
                                          // 기본 프롬프트 지침이 있으면 기본 프롬프트만 유지
                                          setOpenAIInstruction(basicInstruction);
                                        } else {
                                          // 기본 프롬프트 지침도 없으면 모두 제거
                                          setOpenAIInstruction("");
                                        }
                                        setHasPublicGuidelinesInstruction(false);
                                        toast({
                                          title: "지침 제거 완료",
                                          description: "공공기관 방송지침이 제거되었습니다.",
                                        });
                                      }
                                    }}
                                  >
                                    {hasPublicGuidelinesInstruction ? "지침 제거" : "지침으로 추가"}
                                  </Button>
                                </div>
                                <AccordionContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <Tabs value={activeTab} onValueChange={(v) => {
                setActiveTab(v as "manual" | "ai-assist");
                // 직접작성에서 OpenAI 작성으로 이동 시 문구 이전
                if (v === "ai-assist" && customText.trim() && !openAIPrompt.trim()) {
                  setOpenAIPrompt(customText.trim());
                }
                // OpenAI 작성에서 직접작성으로 이동 시 문구 이전
                if (v === "manual" && openAIPrompt.trim() && !customText.trim()) {
                  setCustomText(openAIPrompt.trim());
                }
              }} className="w-full">
                ...
                <TabsContent value="ai-assist" className="space-y-4 mt-4">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label>요청 내용</Label>
                      <Textarea
                        placeholder="예: 폭염 대비 시민 행동요령을 20초 분량으로 작성. 직접 작성 탭에서 작성한 내용이 여기에 표시됩니다."
                        value={openAIPrompt}
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              // 기본 프롬프트 구성
                              let basePrompt = `${purposeLabel}: ${openAIPrompt}`;
                              
                              // 지침이 있으면 기본문구 아래에 추가
                              let instructionPart = "";
                              if (openAIInstruction) {
                                instructionPart = `\n\n${openAIInstruction}`;
                              }
                              
                              // 통합 프롬프트 생성 (검수체크리스트 + 공공기관 방송지침 포함)
                              const optimizedPrompt = generateOptimizedPrompt();
                              const fullPrompt = `${basePrompt}${instructionPart}\n\n${optimizedPrompt}`;
                              
                              let out = await generateWithOpenAI(fullPrompt);
                              
                              // 조사교정 적용
                              out = correctKoreanPostpositions(out);
                              
                              // 300자 초과 시 처리
                              if (out.length > 300) {
                                setSplitTextDialog({ open: true, text: out });
                                return;
                              }
          ...
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription className="text-xs mt-2">
                태그를 클릭하여 해당 태그가 포함된 메시지를 필터링합니다. 드래그하여 회전할 수 있습니다.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {tagCloudData.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground text-sm" style={{ height: '200px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                ...
                <TagCloud3D
                  ...
                  onTagClick={(tag) => setSelectedTagFilter(selectedTagFilter === tag ? "" : tag)}
            ...
            <AlertDialogAction
              onClick={async () => {
                try {
                  ...
                  // 조사교정 적용
                  const corrected = correctKoreanPostpositions(out);
                  setCustomText(corrected);
                  ...
                  toast({
                    title: "축약 완료",
                    description: `문구가 ${corrected.length}자로 축약되었습니다.`,
                  });
                } catch (e: any) {
                  toast({
                    title: "축약 실패",
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                ...
                if (chunks.length > 1) {
                  ...
                  // 조사교정 적용
                  const corrected = correctKoreanPostpositions(combined);
                  setCustomText(corrected);
                  setSplitTextDialog({ open: false, text: "" });
