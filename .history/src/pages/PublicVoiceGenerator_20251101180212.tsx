...
const PublicVoiceGenerator = () => {
  ...
  const startRealtimePreview = async () => {
    ...
    try {
      ...
      if (bgmBuffer) {
        ...
        // BGM 시작 (오프셋 적용)
        const bgmStartTime = ctx.currentTime + Math.max(0, settings.bgmOffset);
        bgmSource.start(bgmStartTime, Math.max(0, -settings.bgmOffset));
      }
      ...
      // 마스터 페이드
      const masterGainValue = masterGain.gain;
      masterGainValue.setValueAtTime(0.0001, ctx.currentTime);
      masterGainValue.exponentialRampToValueAtTime(settings.masterGain, ctx.currentTime + Math.max(0.01, settings.fadeIn));
      
      const ttsEndTime = ctx.currentTime + settings.ttsOffset + ttsBuffer.duration;
      const totalEndTime = settings.trimEndSec ? 
        ctx.currentTime + settings.trimEndSec : 
        Math.max(ttsEndTime, bgmBuffer ? (ctx.currentTime + settings.bgmOffset + bgmBuffer.duration) : ttsEndTime);
      
      masterGainValue.setValueAtTime(settings.masterGain, totalEndTime - Math.max(0.01, settings.fadeOut));
      masterGainValue.exponentialRampToValueAtTime(0.0001, totalEndTime);
  ...
  const loadFavoriteVoices = async () => {
    ...
    try {
      ...
      if (missingVoiceIds.length === 0) return;
      
      console.log(`즐겨찾기된 음성 ${missingVoiceIds.length}개를 로드합니다.`);
      
      // 전체 목록을 로드하여 즐겨찾기 음성들을 찾기
      // 먼저 이미 로드된 allVoices에서 찾기
      const existingFavoriteVoices = allVoices.filter((v: any) => favoriteVoiceIds.has(v.voice_id));
      const stillMissingIds = missingVoiceIds.filter((vid) => 
        !existingFavoriteVoices.find((v: any) => v.voice_id === vid)
      );
      ...
      if (stillMissingIds.length > 0) {
        ...
        if (response?.ok) {
          const data = await response.json();
          const voices = data.items || (Array.isArray(data) ? data : (data.voices || data.data || []));
          const favoriteVoices = voices.filter((v: any) => favoriteVoiceIds.has(v.voice_id));
      ...
      } else if (existingFavoriteVoices.length > 0) {
        // 이미 로드된 음성이 있으면 availableVoices에도 추가
        setAvailableVoices((prev) => {
          const existingIds = new Set(prev.map((v: any) => v.voice_id));
          const newVoices = existingFavoriteVoices.filter((v: any) => !existingIds.has(v.voice_id));
          return [...prev, ...newVoices];
        });
        console.log(`✅ 즐겨찾기 음성 ${existingFavoriteVoices.length}개는 이미 로드되어 있습니다.`);
      }
    } catch (e: any) {
