...
export default function MessageTemplatePage() {
  ...
  const getTemplateExample = (category: string): string => {
    const examples: Record<string, string> = {
      greeting: "안녕하세요. {기관명}입니다. {날짜} {내용}을 안내드립니다. 시민 여러분의 건강과 행복을 기원합니다. 감사합니다.",
      announcement: "{기관명}에서 {내용}을 안내드립니다. 자세한 사항은 {연락처}로 문의해주시기 바랍니다. 많은 관심과 참여 부탁드립니다.",
      policy: "{기관명}의 {정책명} 정책에 대해 안내드립니다. {내용}으로 운영되며, {적용대상}에게 혜택이 제공됩니다. 자세한 사항은 {연락처}로 문의해주시기 바랍니다.",
    };
    return examples[category] || "{기관명}에서 {내용}을 안내드립니다.";
  };

  // 카테고리 변경 시 예시 텍스트 자동 입력
  useEffect(() => {
    if (isCreateDialogOpen && newTemplate.category && newTemplate.category !== "all") {
      if (!newTemplate.text.trim()) {
        setNewTemplate(prev => ({
          ...prev,
          text: getTemplateExample(newTemplate.category),
        }));
      }
    }
  }, [newTemplate.category, isCreateDialogOpen]);

  // 편집 다이얼로그에서도 카테고리 변경 시 예시 텍스트 자동 입력
  useEffect(() => {
    if (isEditDialogOpen && editingTemplate && editingTemplate.templateCategory) {
      if (!editingTemplate.text.trim()) {
        setEditingTemplate(prev => prev ? {
          ...prev,
          text: getTemplateExample(prev.templateCategory || "announcement"),
        } : null);
      }
    }
  }, [editingTemplate?.templateCategory, isEditDialogOpen]);
              ...
              <Select
                ...
                onValueChange={(value) => {
                  setNewTemplate({ 
                    ...newTemplate, 
                    category: value,
                    text: getTemplateExample(value), // 카테고리 선택 시 예시 텍스트 자동 입력
                  });
                }}
              >
                <SelectTrigger>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    setEditingTemplate({ 
                      ...
                      text: getTemplateExample(value), // 카테고리 변경 시 예시 텍스트 자동 입력
                    });
                  }}
                >
                  <SelectTrigger>
