...
export default function MixBoardPage() {
  ...
  const [mixingState, setMixingState] = useState<dbService.MixingStateEntry | null>(null);
    ...
    try {
      ...
      if (state) {
        ...
        setMixingSettings({
          ...
          effectGain: 0,
          masterGain: (state.masterGain || 100) / 100,
          fadeIn: state.fadeIn || 2,
          fadeOut: state.fadeOut || 2,
          fadeInRatio: state.fadeInRatio || 50,
          fadeOutRatio: state.fadeOutRatio || 50,
          lowShelf: state.lowShelf || 0,
          midShelf: state.midShelf || 0,
          highShelf: state.highShelf || 0,
          bgmStartOffset: state.bgmStartOffset || 0,
          bgmOffsetAfterTts: state.bgmOffsetAfterTts || 0,
        });
      }
    } catch (error) {
  ...
  const saveMixingState = async () => {
    ...
    try {
      await dbService.saveMixingState(user.id, {
        ...
        midShelf: mixingSettings.midShelf,
        highShelf: mixingSettings.highShelf,
        bgmStartOffset: mixingSettings.bgmStartOffset,
