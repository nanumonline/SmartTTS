...
const PublicVoiceGenerator = () => {
  ...
  const [templatePage, setTemplatePage] = useState<{ greeting: number; announcement: number; policy: number }>({
    greeting: 1,
    announcement: 1,
    policy: 1,
  });
          ...
          <div className="lg:col-span-1 flex flex-col">
            <Card className="landio-card landio-fade-up flex flex-col h-full">
              ...
              <CardContent className="flex flex-col flex-1 min-h-0">
                ...
                <Tabs defaultValue="greeting" className="w-full flex flex-col flex-1 min-h-0">
                  ...
                  <TabsContent value="greeting" className="flex flex-col flex-1 min-h-0 mt-4">
                    {isLoadingTemplates ? (
                      ...
                      <>
                        <ScrollArea className="flex-1 min-h-0 pr-4">
                          <div className="space-y-3">
                            {(() => {
                              const startIndex = (templatePage.greeting - 1) * templateItemsPerPage;
                              const endIndex = startIndex + templateItemsPerPage;
                              const paginatedTemplates = dbTemplates.greeting.slice(startIndex, endIndex);
                        ...
                        {(() => {
                          const totalPages = Math.ceil(dbTemplates.greeting.length / templateItemsPerPage);
                                ...
                                <PaginationContent>
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, greeting: Math.max(1, prev.greeting - 1) }))}
                                      disabled={templatePage.greeting === 1}
                                      className="gap-1"
                                    >
                                  ...
                                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <PaginationItem key={page}>
                                      <Button
                                        variant={templatePage.greeting === page ? "outline" : "ghost"}
                                        size="sm"
                                        onClick={() => setTemplatePage(prev => ({ ...prev, greeting: page }))}
                                  ...
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, greeting: Math.min(totalPages, prev.greeting + 1) }))}
                                      disabled={templatePage.greeting === totalPages}
                                      className="gap-1"
                                    >
                  ...
                  <TabsContent value="announcement" className="flex flex-col flex-1 min-h-0 mt-4">
                    {isLoadingTemplates ? (
                      <div className="text-center py-8 text-muted-foreground">템플릿 로딩 중...</div>
                      ...
                      <>
                        <ScrollArea className="flex-1 min-h-0 pr-4">
                          <div className="space-y-3">
                            {(() => {
                              const startIndex = (templatePage.announcement - 1) * templateItemsPerPage;
                              const endIndex = startIndex + templateItemsPerPage;
                              const paginatedTemplates = dbTemplates.announcement.slice(startIndex, endIndex);
                              return paginatedTemplates.map((template) => (
                                <Card 
                                  key={template.id}
                                  className={`cursor-pointer transition-all hover:shadow-md ${
                        ...
                        {(() => {
                          const totalPages = Math.ceil(dbTemplates.announcement.length / templateItemsPerPage);
                          if (totalPages <= 1) return null;
                          return (
                            <div className="flex justify-center pt-4 mt-4 border-t">
                              <Pagination>
                                <PaginationContent>
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, announcement: Math.max(1, prev.announcement - 1) }))}
                                      disabled={templatePage.announcement === 1}
                                      className="gap-1"
                                    >
                                      <ChevronLeft className="h-4 w-4" />
                                    </Button>
                                  ...
                                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <PaginationItem key={page}>
                                      <Button
                                        variant={templatePage.announcement === page ? "outline" : "ghost"}
                                        size="sm"
                                        onClick={() => setTemplatePage(prev => ({ ...prev, announcement: page }))}
                                        className="min-w-[2.5rem]"
                                      >
                                        {page}
                                      </Button>
                                  ...
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, announcement: Math.min(totalPages, prev.announcement + 1) }))}
                                      disabled={templatePage.announcement === totalPages}
                                      className="gap-1"
                                    >
                                      <ChevronRight className="h-4 w-4" />
                                    </Button>
                  ...
                  <TabsContent value="policy" className="flex flex-col flex-1 min-h-0 mt-4">
                    {isLoadingTemplates ? (
                      <div className="text-center py-8 text-muted-foreground">템플릿 로딩 중...</div>
                      ...
                      <>
                        <ScrollArea className="flex-1 min-h-0 pr-4">
                          <div className="space-y-3">
                            {(() => {
                              const startIndex = (templatePage.policy - 1) * templateItemsPerPage;
                              const endIndex = startIndex + templateItemsPerPage;
                              const paginatedTemplates = dbTemplates.policy.slice(startIndex, endIndex);
                              return paginatedTemplates.map((template) => (
                                <Card 
                                  key={template.id}
                                  className={`cursor-pointer transition-all hover:shadow-md ${
                        ...
                        {(() => {
                          const totalPages = Math.ceil(dbTemplates.policy.length / templateItemsPerPage);
                          if (totalPages <= 1) return null;
                          return (
                            <div className="flex justify-center pt-4 mt-4 border-t">
                              <Pagination>
                                <PaginationContent>
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, policy: Math.max(1, prev.policy - 1) }))}
                                      disabled={templatePage.policy === 1}
                                      className="gap-1"
                                    >
                                      <ChevronLeft className="h-4 w-4" />
                                    </Button>
                                  ...
                                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <PaginationItem key={page}>
                                      <Button
                                        variant={templatePage.policy === page ? "outline" : "ghost"}
                                        size="sm"
                                        onClick={() => setTemplatePage(prev => ({ ...prev, policy: page }))}
                                        className="min-w-[2.5rem]"
                                      >
                                        {page}
                                      </Button>
                                  ...
                                  <PaginationItem>
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setTemplatePage(prev => ({ ...prev, policy: Math.min(totalPages, prev.policy + 1) }))}
                                      disabled={templatePage.policy === totalPages}
                                      className="gap-1"
                                    >
                                      <ChevronRight className="h-4 w-4" />
                                    </Button>
