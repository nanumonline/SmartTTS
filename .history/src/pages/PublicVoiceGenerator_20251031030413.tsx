...
const PublicVoiceGenerator = () => {
  ...
  // 드롭다운 옵션
  const languageOptions = ["ko", "en", "ja", "zh", "es", "fr", "de"];
  const styleOptions = ["neutral", "happy", "sad", "angry", "calm", "friendly", "professional"];
  const genderOptions = ["male", "female", "neutral"];
  const useCaseOptions = ["announcement", "public-service", "broadcast", "education", "marketing"];

  // Supertone API 엔드포인트 (공식 레퍼런스: https://docs.supertoneapi.com/en/api-reference/introduction)
  const SUPABASE_PROXY_BASE_URL = import.meta.env.DEV
    ? "http://localhost:54321/functions/v1/supertone-proxy"
    : "https://gxxralruivyhdxyftsrg.supabase.co/functions/v1/supertone-proxy";
  ...
  const searchVoices = useCallback(async () => {
    ...
    try {
      ...
      if (response?.ok) {
        const data = await response.json();
        const results = Array.isArray(data) ? data : data.voices || data.data || [];
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-4">
            <div className="md:col-span-2 space-y-3">
              <div>
                ...
                <Select value={voiceFilters.language} onValueChange={(v) => setVoiceFilters(prev => ({ ...prev, language: v }))}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={voiceFilters.style} onValueChange={(v) => setVoiceFilters(prev => ({ ...prev, style: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="전체" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                {/* 이름 빠른 선택 */}
                <div className="mt-2">
                  <Select value="" onValueChange={(v) => setVoiceFilters(prev => ({ ...prev, name: v }))}>
                    <SelectTrigger>
                      <SelectValue placeholder="이름 빠른 선택 (옵션)" />
                    </SelectTrigger>
                    <SelectContent>
                      {(availableVoices || []).slice(0, 100).map((v: any) => (
              ...
              <div>
                ...
                <Select value={voiceFilters.gender} onValueChange={(v) => setVoiceFilters(prev => ({ ...prev, gender: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="전체" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select value={voiceFilters.useCase} onValueChange={(v) => setVoiceFilters(prev => ({ ...prev, useCase: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="전체" />
                  </SelectTrigger>
                  <SelectContent>
