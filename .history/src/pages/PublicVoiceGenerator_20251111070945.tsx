...
const PublicVoiceGenerator = () => {
  ...
  const pushHistory = async (entry: any): Promise<any> => {
    ...
    try {
      ...
      // 로컬 상태 업데이트 (cacheKey와 mimeType 포함, audioUrl은 null로 설정)
      const savedEntry = { 
        ...entry, 
        ...
        audioUrl: null, // blob URL은 생성 이력에 포함하지 않음 (필요할 때만 생성)
      };
  ...
  const proceedWithGeneration = async (textToUse: string) => {
    ...
    try {
      ...
      // blob URL 생성 (필요시 재생성하도록 cacheRef에 저장)
              ...
              {generationHistory.length === 0 ? (
                ...
                <div className="space-y-3">
                  {generationHistory.map((entry) => {
                    ...
                    // audioUrl은 렌더링 시 생성하지 않음 (필요할 때만 생성)
                    // blob URL은 미리듣기 클릭 시 또는 AudioPlayer 렌더링 시에만 생성
                    let audioUrl = entry.audioUrl;
                    
                    // blob: URL은 null로 설정 (렌더링 시 생성하지 않음)
                    if (audioUrl && audioUrl.startsWith('blob:')) {
                      audioUrl = null;
                    }
                    
                    // 외부 URL만 사용 (blob URL은 제외)
                    if (audioUrl && !audioUrl.startsWith('blob:')) {
                      // 외부 URL은 그대로 사용
                    } else {
                      audioUrl = null; // blob URL은 null로 설정하여 렌더링 시 생성하지 않음
                    }
                    return (
