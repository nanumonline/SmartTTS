...
const PublicVoiceGenerator = () => {
  ...
  const computeVoiceLanguageRank = (voice: any): number => {
    ...
    for (let i = 0; i < LANGUAGE_PRIORITY.length; i++) {
      if (langs.includes(LANGUAGE_PRIORITY[i])) return i;
    }
  ...
  const searchVoices = useCallback(async () => {
    ...
    try {
      ...
      if (response?.ok) {
        ...
        // 배경 로딩(완화된 속도 / 최대 5페이지)
        if (nextToken) { autoLoadVoicesThrottled(5, 300); }
      } else if (response) {
  ...
  const applyClientFilters = (voices: any[], filters: typeof voiceFilters) => {
    const filtered = voices.filter((v) => {
      ...
      // 용도
      if (filters.useCase) {
        if ((v.use_case || v.useCase || "") !== filters.useCase) return false;
      }
      return true;
    });
                    ...
                    <SelectContent>
                      ...
                      {availableVoices.length > 0 ? (
                        [...availableVoices]
                          ...
                          // 첫 아이템에 헤더 추가
                          .map((voice: any, i: number) => {
                            const header = i === 0 ? (
                              <div key="header" className="px-2 py-1 text-[11px] text-muted-foreground grid grid-cols-5 gap-2">
                                <div>즐겨찾기</div>
                                <div>성별</div>
                                <div>이름</div>
                                <div>국가</div>
                              ...
                              <div key={voice.voice_id + "-wrap"}>
                                ...
                                <SelectItem value={voice.voice_id}>
                                  <div className="grid grid-cols-5 gap-2 items-center">
                                    ...
                                    <div className="truncate text-sm font-medium">{voiceName}</div>
                                    <div className="text-xs">{flags}</div>
                                    <div className="text-xs truncate">{stylesKo || '-'}</div>
                                  </div>
                                </SelectItem>
                              </div>
                            );
