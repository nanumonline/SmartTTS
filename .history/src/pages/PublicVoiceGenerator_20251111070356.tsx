              ...
              {generationHistory.length === 0 ? (
                ...
                <div className="space-y-3">
                  {generationHistory.map((entry) => {
                    ...
                    // audioUrl이 없거나 cross-origin blob URL인 경우 DB에서 재로드 시도
                    if (!audioUrl && user?.id && entry.id && entry.cacheKey) {
                      // 백그라운드에서 재로드 시도 (에러는 조용히 처리)
                      dbService.loadGenerationBlob(user.id, String(entry.id))
                        .then((blobData) => {
                          if (blobData?.audioBlob) {
