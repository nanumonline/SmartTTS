...
const PublicVoiceGenerator = () => {
  ...
  const concatenateAudios = async (audioBlobs: Blob[], preserveFormat: boolean = false): Promise<Blob> => {
    ...
    if (preserveFormat) {
      ...
      if (allMp3) {
        // mp3 파일을 그대로 결합 (간단한 바이너리 결합)
        // 주의: 이 방법은 완벽하지 않지만 대부분의 경우 작동합니다
      ...
      if (audioChunks.length > 1) {
        ...
        if (allMp3) {
          try {
            ...
            finalMimeType = "audio/mpeg"; // mp3 형식 유지
            console.log("✅ MP3 형식으로 결합 완료");
          } catch (error) {
            console.warn("MP3 결합 실패, WAV로 변환:", error);
            // 실패 시 WAV로 변환
