...
export async function exportMixToWav(
  ...
  mg.setValueAtTime(0.0001, 0);
  mg.exponentialRampToValueAtTime(settings.masterGain, Math.max(0.01, settings.fadeIn));
  mg.setValueAtTime(settings.masterGain, renderDur - Math.max(0.01, settings.fadeOut));
  mg.exponentialRampToValueAtTime(0.0001, renderDur);
