...
export async function exportMixToWav(
  ...
  // BGM 전체 길이: trimEndSec가 있으면 그것이 BGM 전체 길이 (TTS 종료 후 연장 포함)
  ...
  if (bgmBuffer && settings.trimEndSec != null && settings.trimEndSec > 0) {
    ...
    // trimEndSec가 없으면 TTS 길이 + fadeIn + fadeOut로 확장
    const minBgmLen = ttsLen + (settings.fadeIn || 0) + (settings.fadeOut || 0);
    const bgmOriginalLen = bgmStartTime + bgmBuffer.duration;
    bgmTotalLen = Math.max(minBgmLen, bgmOriginalLen);
  }
