...
import { Plus, FileText, Edit, Trash2, Copy, CheckCircle, Save, ChevronLeft, ChevronRight, AlertCircle } from "lucide-react";
...
export default function MessageTemplatePage() {
  ...
  const getTemplateExample = (category: string): string => {
    const examples: Record<string, string> = {
      ...
      environment: "{기관명}에서 환경 정책을 안내드립니다. {정책명}을 통해 {정책목표}를 달성하고자 합니다. 시민 여러분의 {참여방법} 참여를 부탁드리며, {기대효과}를 기대하고 있습니다. 자세한 사항은 {연락처}로 문의해주세요.",
        ...
        <AlertDialogContent className="sm:max-w-[500px]">
          <AlertDialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <AlertDialogTitle className="text-xl">템플릿 삭제 확인</AlertDialogTitle>
            </div>
            <AlertDialogDescription className="text-base pt-2">
              {(() => {
                const template = deleteDialog.id ? templates.find((t) => t.id === deleteDialog.id) : null;
                if (template) {
                  return (
                    <div className="space-y-3">
                      ...
                      <div className="p-4 bg-muted/50 rounded-lg border border-border space-y-2">
                        {template.templateName && (
                          <div className="flex items-start gap-2">
                            ...
                            <span className="text-sm font-semibold flex-1">{template.templateName}</span>
                          </div>
                        )}
                        {template.templateCategory && (
                          <div className="flex items-start gap-2">
                            <span className="text-sm font-medium text-muted-foreground min-w-[60px]">카테고리:</span>
                            <Badge variant="outline" className="text-xs">{getCategoryLabel(template.templateCategory)}</Badge>
                          </div>
                        )}
                        {template.purpose && (
                          <div className="flex items-start gap-2">
                            <span className="text-sm font-medium text-muted-foreground min-w-[60px]">목적:</span>
                            <Badge variant="outline" className="text-xs">{getPurposeLabel(template.purpose)}</Badge>
                          </div>
                        )}
                        {template.text && (
                          <div className="flex items-start gap-2">
                            <span className="text-sm font-medium text-muted-foreground min-w-[60px]">내용:</span>
                            <span className="text-sm flex-1 line-clamp-3">{template.text}</span>
                          </div>
                        )}
                      </div>
                    </div>
